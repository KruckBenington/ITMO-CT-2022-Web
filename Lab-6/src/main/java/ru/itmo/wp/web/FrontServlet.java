...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method method = getMethod(route, pageClass, route.getAction());
        Method methodBefore = getMethod(route, pageClass, "before");
        Method methodAfter = getMethod(route, pageClass, "after");

        for (Class<?> clazz = pageClass; methodAfter == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                methodAfter = clazz.getDeclaredMethod("after", HttpServletRequest.class, Map.class);
    ...
    private Method getMethod(Route route, Class<?> pageClass, String actionName) {
        Method method = null;
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                method = clazz.getDeclaredMethod(actionName, HttpServletRequest.class, Map.class);
            } catch (NoSuchMethodException ignored) {
                // No operations.
            }
        }
